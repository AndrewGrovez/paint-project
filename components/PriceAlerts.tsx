...
export default function PriceAlerts() {
  ...
  const markAsNotified = async (alertId: string) => {
    try {
      console.log('Attempting to mark alert as notified:', alertId)
      ...
      console.log('Successfully marked alert as notified:', alertId)
      // Re-fetch alerts to update UI
      await fetchAlerts()
    } catch (error) {
      console.error('Error marking alert as notified:', error)
      // Fallback: Optimistically remove the alert from local state if fetch fails
        ...
        {alerts.map((alert) => (
          ...
          >
            <div className="flex justify-between items-start">
              ...
              <button
                onClick={() => {
                  console.log('Dismiss button clicked for alert:', alert.id)
